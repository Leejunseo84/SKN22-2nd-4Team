...
- **타겟 변수**: `churn` (이탈 여부)
- **초기 상태**: 약 **14.1%**의 고객이 이탈한 불균형 데이터셋.
...
- **최종 선택: 라벨 인코딩 (Label Encoding)**
- **사유**: state(주)나 area_code(지역 번호)와 같이 범주가 너무 많아 모델의 복잡도를 과하게 높일 수 있는 변수들은 라벨 인코딩(Label Encoding)을 거쳐 포함시켰습니다.
